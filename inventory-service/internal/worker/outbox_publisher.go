...
// OutboxStore defines the persistence capabilities required by the worker.
...
func (w *OutboxPublisherWorker) Start(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			if err := w.processBatchSafe(ctx); err != nil {
				w.logger.Error("OutboxPublisherWorker polling cycle error: %v", err)
			}
		}
	}
}

// Stop signals the worker to stop and waits for in-flight processing to complete.
	...
	err := w.transactor.WithTransaction(ctx, func(txCtx context.Context) error {
		...
		for _, evt := range events {
			if evt.RetryCount >= w.maxRetries {
				w.logger.Warn("OutboxPublisherWorker event %s exceeded max retries (%d), skipping", evt.ID, w.maxRetries)
				continue
			}
		...
		if len(publishedIDs) > 0 {
			if err := w.repo.MarkPublished(txCtx, publishedIDs); err != nil {
				return fmt.Errorf("marking events published: %w", err)
			}
		}
